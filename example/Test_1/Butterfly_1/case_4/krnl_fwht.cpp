...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    float prev_buffer_4[FEATURES/2];
    ...
    float prev_buffer_6[FEATURES/8];
    ...
    for (unsigned pe = 0; pe < kComputeTiles/(2*8); pe++)
    {
        ...
        {
            #pragma HLS UNROLL
            Butterfly0_V1(pipe0[index],pipe1[index],8,index*8,prev_buffer_1);
            index++;
        }
        for (int BLOCK_R = 0; BLOCK_R < 8; BLOCK_R++)
        {
            #pragma HLS UNROLL
            Butterfly1_V1(pipe0[index],pipe1[index],8,index*8,prev_buffer_1);
            index++;
        }
    }
    //U(3)BU^-1(3)
    ...
    for (unsigned pe = 0; pe < kComputeTiles/(2*4); pe++)
    {
        ...
        {
            #pragma HLS UNROLL
            Butterfly0_V1(pipe1[index],pipe2[index],4,index*4,prev_buffer_2);
            index++;
        }
        for (int BLOCK_R = 0; BLOCK_R < 4; BLOCK_R++)
        {
            #pragma HLS UNROLL
            Butterfly1_V1(pipe1[index],pipe2[index],4,index*4,prev_buffer_2);
            index++;
        }
    }
    //U(2)BU^-1(2)
    ...
    for (unsigned pe = 0; pe < kComputeTiles/(2*2); pe++)
    {
        ...
        {
            #pragma HLS UNROLL
            Butterfly0_V1(pipe2[index],pipe3[index],2,index*2,prev_buffer_3);
            index++;
        }
        for (int BLOCK_R = 0; BLOCK_R < 2; BLOCK_R++)
        {
            #pragma HLS UNROLL
            Butterfly1_V1(pipe2[index],pipe3[index],2,index*2,prev_buffer_3);
            index++;
        }
    }
    ...
    for (unsigned pe = 0; pe < kComputeTiles/(2*8); pe++)
    {
        ...
        {
            #pragma HLS UNROLL
            Butterfly0_V1(pipe8[index],pipe9[index],8,index*8,prev_buffer_4);
            index++;
        }
        for (int BLOCK_R = 0; BLOCK_R < 8; BLOCK_R++)
        {
            #pragma HLS UNROLL
            Butterfly2_V1(pipe8[index],pipe9[index],8,index*8,prev_buffer_4);
            index++;
        }
    }
    //U(3)BR(3)U^-1(3)
    ...
    for (unsigned pe = 0; pe < kComputeTiles/(2*4); pe++)
    {
        ...
        {
            #pragma HLS UNROLL
            Butterfly0_V1(pipe9[index],pipe10[index],4,index*4,prev_buffer_5);
            index++;
        }
        for (int BLOCK_R = 0; BLOCK_R < 4; BLOCK_R++)
        {
            #pragma HLS UNROLL
            Butterfly2_V1(pipe9[index],pipe10[index],4,index*4,prev_buffer_5);
            index++;
        }
    }
    //U(2)BU^-1(2)
    ...
    for (unsigned pe = 0; pe < kComputeTiles/(2*2); pe++)
    {
        ...
        {
            #pragma HLS UNROLL
            Butterfly0_V1(pipe10[index],pipe11[index],2,index*2,prev_buffer_6);
            index++;
        }
        for (int BLOCK_R = 0; BLOCK_R < 2; BLOCK_R++)
        {
            #pragma HLS UNROLL
            Butterfly2_V1(pipe10[index],pipe11[index],2,index*2,prev_buffer_6);
            index++;
        }
    }
    
    //U(1)BR(1)U^-1(1)
    last_URBU_V1(pipe11, out_stream_0);
