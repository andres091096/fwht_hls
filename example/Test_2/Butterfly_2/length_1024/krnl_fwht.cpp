...
#pragma HLS dataflow
    ...
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe0[pe],pipe7[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_1_1,prev_buffer_1_2);
    }
    //U(8)BU^-1(8)
    ...
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe7[pe],pipe8[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_2_1,prev_buffer_2_2);
    }
    //U(7)BU^-1(7)
    ...
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe8[pe],pipe9[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_3_1,prev_buffer_3_2);
    }
    //U(6)BU^-1(6)
    ...
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe9[pe],pipe10[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_4_1,prev_buffer_4_2);
    }
    //U(5)BU^-1(5)
    ...
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe10[pe],pipe11[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_5_1,prev_buffer_5_2);
    }
    //U(4)BU^-1(4)
    FIFO_SIZE    = 8; // = 2^(U_k-1)
    for (unsigned pe = 0; pe < kComputeTiles; pe++)
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe11[pe],pipe12[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_6_1,prev_buffer_6_2);
    }
    //U(3)BU^-1(3)
    FIFO_SIZE    = 4; // = 2^(U_k-1)
    for (unsigned pe = 0; pe < kComputeTiles; pe++)
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe12[pe],pipe13[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_7_1,prev_buffer_7_2);
    }
    //U(2)BU^-1(2)
    FIFO_SIZE    = 2; // = 2^(U_k-1)
    for (unsigned pe = 0; pe < kComputeTiles; pe++)
    {
        #pragma HLS UNROLL
        Butterfly0_V2(pipe13[pe],pipe14[pe],FIFO_SIZE,pe*FIFO_SIZE,prev_buffer_8_1,prev_buffer_8_2);
    }
    //U(1)BU^-1(1)
